...
constructor(
    ...
    private fun findClassDescriptor(
            ...
            location: LookupLocation): ClassDescriptor {
    ...
    private fun resolveToDescriptor(declaration: KtDeclaration, track: Boolean): DeclarationDescriptor {
        val result = declaration.accept<DeclarationDescriptor, Unit>(object : KtVisitor<DeclarationDescriptor, Unit>() {
            ...
            override fun visitClass(klass: KtClass, data: Unit): DeclarationDescriptor {
                return getClassDescriptor(klass, lookupLocationFor(klass, klass.isTopLevel()))
            }

            override fun visitObjectDeclaration(declaration: KtObjectDeclaration, data: Unit): DeclarationDescriptor {
                return getClassDescriptor(declaration, lookupLocationFor(declaration, declaration.isTopLevel()))
            }

            override fun visitTypeParameter(parameter: KtTypeParameter, data: Unit): DeclarationDescriptor {
            ...
            override fun visitNamedFunction(function: KtNamedFunction, data: Unit): DeclarationDescriptor {
                ...
                return bindingContext.get(BindingContext.DECLARATION_TO_DESCRIPTOR, function)
                       ?: error("No descriptor found for named function ${function.text}")
            }

            override fun visitParameter(parameter: KtParameter, data: Unit): DeclarationDescriptor {
                ...
                if (grandFather is KtPrimaryConstructor) {
                    ...
                    if (parameter.hasValOrVar()) {
                        ...
                        return bindingContext.get(BindingContext.PRIMARY_CONSTRUCTOR_PARAMETER, parameter)
                               ?: error("No descriptor found for property parameter ${parameter.text} of constructor ${grandFather.text}")
                    }
                    else {
                        ...
                        return bindingContext.get(BindingContext.VALUE_PARAMETER, parameter)
                               ?: error("No descriptor found for value parameter ${parameter.text} of constructor ${grandFather.text}")
                    }
                }
                    ...
                    return bindingContext.get(BindingContext.VALUE_PARAMETER, parameter)
                           ?: error("No descriptor found for value parameter ${parameter.text} of function ${grandFather.text}")
                }
                else if (grandFather is KtSecondaryConstructor) {
                    val constructorDescriptor = visitSecondaryConstructor(
                            grandFather, data) as ConstructorDescriptor
                    constructorDescriptor.valueParameters
                    return bindingContext.get(BindingContext.VALUE_PARAMETER, parameter)
                           ?: error("No descriptor found for value parameter ${parameter.text} of secondary constructor ${grandFather.text}")
                }
                else {
            ...
            override fun visitSecondaryConstructor(constructor: KtSecondaryConstructor, data: Unit): DeclarationDescriptor {
                ...
                return bindingContext.get(BindingContext.CONSTRUCTOR, constructor)
                       ?: error("No descriptor found for secondary constructor ${constructor.text}")
            }

            override fun visitPrimaryConstructor(constructor: KtPrimaryConstructor, data: Unit): DeclarationDescriptor {
                getClassDescriptor(constructor.getContainingClassOrObject(), lookupLocationFor(constructor, false)).constructors
                return bindingContext.get(BindingContext.CONSTRUCTOR, constructor)
                       ?: error("No descriptor found for primary constructor ${constructor.text}")
            }

            override fun visitProperty(property: KtProperty, data: Unit): DeclarationDescriptor {
                ...
                return bindingContext.get(BindingContext.DECLARATION_TO_DESCRIPTOR, property)
                       ?: error("No descriptor found for property ${property.text}")
            }

            override fun visitTypeAlias(typeAlias: KtTypeAlias, data: Unit): DeclarationDescriptor {
                ...
                return bindingContext.get(BindingContext.DECLARATION_TO_DESCRIPTOR, typeAlias)
                       ?: error("No descriptor found for type alias ${typeAlias.text}")
            }

            override fun visitScript(script: KtScript, data: Unit): DeclarationDescriptor {
                return getScriptDescriptor(script, lookupLocationFor(script, true))
            }

            override fun visitKtElement(element: KtElement, data: Unit): DeclarationDescriptor {
